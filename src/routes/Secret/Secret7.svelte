...
<div class="wrapper">
    <div class="left">
        ...
        <div class="log">
            {JSON.stringify(getChangedSubmit())}
            ...
            <div class="tableCls">
                <table>
                    <tr>
                        ...
                        <th style="width: 130px">Sn</th>
                        <th style="width: 50px">Chr</th>
                        <th style="width: 130px">posStart</th>
                        <th style="width: 130px">posEnd</th>
                        <th style="width: 130px">ref </th>
                        <th style="width: 130px">alt </th>
                        <th style="width: 50px">Delete</th>
                    </tr>
                    <!-- TODO 此处each标签如果添加(item.id)就会i保持很奇怪的维持上一状态，全乱了 注意不要乱加，这是弊端
                         但是添加了后，又能获得通过document.getElementById('del'+item.id).value获得值，不然获取不到-->
                    {#each pageData() as item, i (item.id)}
                        <tr class="{nowResultId===item.id?'now':''} {submitDict[item.id].type}"
                            on:click={()=>changeTargetId(item.sn, item.id)}>
                            <td>
                                <span class="{submitDict[item.id].type===statusTypes.LOCKED?'icon-lock':'icon-unlocked'}"
                                    on:click={()=>handleLockStatusInSubmitData(item.id)}></span>
                            </td>
                            <td>{item.sn}</td>
                            <td>{item.chr}</td>
                            <td>
                                <input type="number" bind:value="{item.posStart}" disabled="{submitDict[item.id].type===statusTypes.LOCKED||submitDict[item.id].type===statusTypes.DELETED?'true':''}" />
                                <span class="{checkAttrValueChanging(attrTypes.POSSTART, item.id)?'icon-undo2':''}"
                                    on:click|stopPropagation={()=>handleRecoverValue(attrTypes.POSSTART,item.id)}></span>
                            </td>
                            <td>
                                <input type="number" bind:value="{item.posEnd}" disabled="{submitDict[item.id].type===statusTypes.LOCKED||submitDict[item.id].type===statusTypes.DELETED?'true':''}" />
                                <span class="{checkAttrValueChanging(attrTypes.POSEND, item.id)?'icon-undo2':''}"
                                      on:click|stopPropagation={()=>handleRecoverValue(attrTypes.POSEND,item.id)}></span>
                            </td>
                            <td class="refalt">
                                <input type="text" bind:value="{item.ref}" disabled="{submitDict[item.id].type===statusTypes.LOCKED||submitDict[item.id].type===statusTypes.DELETED?'true':''}" />
                                <span class="{checkAttrValueChanging(attrTypes.REF, item.id)?'icon-undo2':''}"
                                      on:click|stopPropagation={()=>handleRecoverValue(attrTypes.REF,item.id)}></span>
                            </td>
                            <td class="refalt">
                                <input type="text" bind:value="{item.alt}" disabled="{submitDict[item.id].type===statusTypes.LOCKED||submitDict[item.id].type===statusTypes.DELETED?'true':''}" />
                                <span class="{checkAttrValueChanging(attrTypes.ALT, item.id)?'icon-undo2':''}"
                                      on:click|stopPropagation={()=>handleRecoverValue(attrTypes.ALT,item.id)}></span>
                            </td>
<!--                            这里利用bind:value的双引号，将boolen转化为string类型，从而可以判断前后差异-->
                            <td>
                                <div class="beDelWrapper">
                                    <div class="bnDel">
                                        <span class="{submitDict[item.id].type===statusTypes.DELETED?'icon-cross':'icon-cross grey'}"
                                            on:click|stopPropagation={()=>handleDeleteInSubmitData(item.id)}></span>
...
<script>
    ...
    let statusTypes = {'FREE': 'free', 'CHECKED': 'checked', 'LOCKED': 'locked', 'DELETED': 'deleted', 'EDITED': 'edited'}
    let attrTypes = {'POSSTART': 'posStart', 'POSEND': 'posEnd', 'REF': 'ref', 'ALT': 'alt'}
    ...
    $: getChangedSubmit = () => {
        let changedDict = {}
        for (let attr in submitDict) {
            let submit = submitDict[attr]
            let type = submit.type
            if (type!==statusTypes.FREE) {
                changedDict[attr] = submitDict[attr]
            }
        }
        return changedDict
    }
    ...
    // 《测试区域《测试区域《测试区域《测试区域《测试区域《测试区域《测试区域《测试区域《测试区域
    ...
    function cho(fnName){
        if (fnName===minus.name) return (a,b)=>minus(a,b)
        if (fnName===plus.name) return (a,b)=>plus(a,b)
    }
    function test(){
        // add()
        ...
        // cho('minus')(1,2)
        logs = []
        console.log(JSON.stringify(submitDict))
        logs = logs
    }
    // 测试区域》测试区域》测试区域》测试区域》测试区域》测试区域》测试区域》测试区域》测试区域》
        ...
        for (let result of data) {
            for (let item of result.results) {
                if (item.id === resultId) {
                    ...
                    // 先判断是否锁住，没锁才能修改
                    // 判断是否还存在其它的修改，如果没有edited->checked
                    if (submitDict[resultId].type !== statusTypes.LOCKED) {
                        // 删除submitData表中数据
                        delete submitDict[resultId].content[attr]
                        let leftNum = Object.getOwnPropertyNames(submitDict[resultId].content).length
                        // console.log('oldVal: leftNum ' + leftNum)
                        if (leftNum === 0) {
                            submitDict[resultId].type = statusTypes.CHECKED
                        }

                        // 数据驱动视图变化，必须要这样才能响应
                        // 因为检测的是data和copyData的对应位置值是否相等
                        data = data
                    }
                    return
                }
    ...
    // 检测data和copyData中某属性（attr）数据是否有修改
    ...
    // 有前提，以attrTypes表中属性字段为依据
    //toObj中有这个属性，fromObj中没有
    function updateAttrValuesInSubmit (toObj, fromObj) {
        for (let attr in attrTypes) {
            // from中有，to中有->修改 // from中有，to中没有-> 添加
            // from中没有，to中有->删除
            console.log("updateAttrValue: " + toObj[attrTypes[attr]] + " " + fromObj[attrTypes[attr]])
            //空值和NULL不一样
            if (fromObj[attrTypes[attr]]!=='' || fromObj[attrTypes[attr]]!==undefined) {
                toObj[attrTypes[attr]] = fromObj[attrTypes[attr]]
            }else{
                delete toObj[attrTypes[attr]]
            }
        }
    }
    ...
    function changeTargetId(sn, resultId){
        ...
        if (nowResultId !== resultId) {
            // 对上一resultID即nowResultID的copyData和data的4个attr上值进行比较
            if (nowResultId) {
                let type = submitDict[nowResultId].type
                let obj = {}
                if (checkAttrValueChanging(attrTypes.POSSTART, nowResultId)) obj[attrTypes.POSSTART] = valueOfResultId(data, attrTypes.POSSTART, nowResultId)
                if (checkAttrValueChanging(attrTypes.POSEND, nowResultId)) obj[attrTypes.POSEND] = valueOfResultId(data, attrTypes.POSEND, nowResultId)
                if (checkAttrValueChanging(attrTypes.REF, nowResultId)) obj[attrTypes.REF] = valueOfResultId(data, attrTypes.REF, nowResultId)
                if (checkAttrValueChanging(attrTypes.ALT, nowResultId)) obj[attrTypes.ALT] = valueOfResultId(data, attrTypes.ALT, nowResultId)
                console.log(JSON.stringify(obj) + " " + type)
                if (JSON.stringify(obj)==='{}') {
                    // 表示上一条记录没有发生修改，free->checked, locked->no , deleted->no, checked->no, edited(不可能)
                    console.log("obj==={}")
                    if (type===statusTypes.FREE) submitDict[nowResultId].type = statusTypes.CHECKED
                }else{
                    // free->edited, locked->no, deleted不可能, checked->edited, edited->edited
                    console.log("obj不等于{}")
                    if (type===statusTypes.FREE || type===statusTypes.CHECKED || type===statusTypes.EDITED){
                        submitDict[nowResultId].type = statusTypes.EDITED
                        //修改submit表中记录
                        updateAttrValuesInSubmit(submitDict[nowResultId].content, obj)
                    }
                }
            }
            ...
            // console.log("changeTargetId: posStart " + checkAttrValueChanging(attrTypes.POSSTART, nowResultId))
    ...
    function addSn(snIndex){
        if (selectedSnIndex.indexOf(snIndex) === -1){
            ...
            // 按数值大小排序
            selectedSnIndex = selectedSnIndex.sort((a,b)=>{return a-b})
            // console.log(selectedSnIndex)
            // 页面溢出矫正
            loadTracks()
        }else{
    ...
    //仅在onMount时使用
    function buildSubmitDict(data) {
        for (let item of data) {
            for (let result of item.results) {
                submitDict[result.id] = {'type': result.delete?statusTypes.DELETED:statusTypes.FREE , 'content': {}}
            }
        }
    }

    function handleLockStatusInSubmitData(resultId) {
        let type = submitDict[resultId].type
        if (type!==statusTypes.LOCKED){
            submitDict[resultId].type = statusTypes.LOCKED
        }else{
            // console.log(submitDict[resultId].content) //居然{}是object object，无法用于判断空
            // 错误 TODO submitDict[resultId].type = submitDict[resultId].content?statusTypes.EDITED:statusTypes.FREE
            submitDict[resultId].type = JSON.stringify(submitDict[resultId]) === '{}'?statusTypes.EDITED:statusTypes.FREE
        }
    }

    function handleDeleteInSubmitData(resultId) {
        let type = submitDict[resultId].type
        if (type===statusTypes.LOCKED) return
        if (type!==statusTypes.DELETED) {
            for (let attr in attrTypes) {
                // 清空submit表中数据
                delete submitDict[resultId].content[attrTypes[attr]]
                // 所有data表中所有值修改回去, 类似于点击恢复按钮
                handleRecoverValue(attrTypes[attr], resultId)
            }
            // 修改状态
            submitDict[resultId].type = statusTypes.DELETED
        }else{
            submitDict[resultId].type = statusTypes.FREE
        }
    }
    ...
    buildSubmitDict(data)
    console.log(submitDict)

    //下载数据后，建立submitDict字典
</script>
