    ...
    <div class="left">
        ...
        {#each data as item, i (item.sn)}
            <div class="{selectedSnId.indexOf(i)>-1?'inSelectedSnList':''}">
                <button on:click={()=>changeSn(item.sn, i)}>{item.sn}</button>
...
<!--                <span class="{selectedSnId.indexOf(i)}"></span>-->
    ...
    <div class="right">
        <div class="rightUp">
            ...
            <button>TMB</button>
<!--            // 这里面的add后括号一定不能省旅-->
            <button on:click={()=>add()}>TRY</button>{count} {addCount}
        </div>
        ...
        <div>
            <table class="tableCls">
                ...
                {#each data[nowSnId].result as item, i}
                    <tr class="{nowTargetId===i?'now':''}"
                        on:click={()=>changeNowTgetIdandLoucs(i)}>
                        <td>{i+1}</td>
                        <td>{item.chr}</td>
                        <td>{item.posStart}</td>
                        <td>{item.posEnd}</td>
...
<script>
    ...
    function changeNowTgetIdandLoucs(i) {
        nowTargetId = i
        for (let track of trackConfigs) {
            console.log(track.name)
            console.log(data[nowSnId].sn)
            if (track.name.indexOf(data[nowSnId].sn) > -1 && igvBrowser) {
                changeLocus()
            }
        }
    ...
    function load() {
        ...
        fileWidget.value = ''
        console.log("load nowSnId: " + nowSnId)
        let sn = data[nowSnId].sn
        loadOnlyTrack(sn)
        changeLocus()
    }
