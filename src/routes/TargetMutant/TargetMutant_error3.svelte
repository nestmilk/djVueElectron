...
<!--                            bind:this={uploadMessageDiv}-->
                            ...
                            <div class="uploadBnWrapper">
                                <button class="icon-cloud-upload" on:click={()=>handleEventForSure(model_dict.PANAL, dict.SUBMIT)}>&nbsp;提交</button>
                                <button class="icon-download" on:click={()=>download()}>&nbsp;下载</button>
                            </div>
                        </div>
...
<!--                    bind:this={topScroll}-->
                    <div class="topInsideWrapper" on:scroll={()=>handleScroll(dict.TOPSCROLL)}>
...
<!--                    bind:this={insideScroll}-->
                    <div class="insideWrapper"  on:scroll={()=>handleScroll(dict.INSIDESCROLL)}>
                        <table class="rightMutantTable">
                            {#each mutant_list as mutant (mutant.id)}
                                <tr class="{cur_mutant_id===mutant.id?'current':''} {mutant_submit_dict[mutant.id]?
                                    ...
                                    on:click={()=>handleChangeCurrentMutantId(mutant.id)}>
                                    <td class="done">
...
{#if sureShow}
    ...
    <script>
        ...
        let dict = {CONTENT: 'content', GET: 'get', ADD: 'add', MODIFY: 'modify', SUBMIT: 'submit', DELETE: 'delete',
                ...
                case model_dict.PANAL:
                    switch (sureEvent) {
                        case dict.SUBMIT:
                            ...
                            console.log(totalSubmitAndAffirmed + " " + mutant_num)
                            if (totalSubmitAndAffirmed === mutant_num) {
                                errors.detail = '此Panal已经全部提交，可直接下载'
                                return
                            }
                            if (totalUnsubtmitAndAffirmed === 0) {
                                errors.detail = '您没有需要待提交的项目'
                                return
                            }
