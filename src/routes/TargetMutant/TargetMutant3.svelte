            ...
            <div class="contentRight">
                <div class="mutantList">
                    ...
                    <div class="insideWrapper" bind:this={insideScroll} on:scroll={()=>handleScroll(dict.INSIDESCROLL)}>
                        <table class="rightMutantTable">
                            {#each mutant_list as mutant (mutant.id)}
                                <tr class="{now_mutant_id===mutant.id?'current':''} {mutant_submit_dict[mutant.id]?
                                                                                        (mutant_submit_dict[mutant.id][dict.STATUS] === mutant_status_dict.FREE && mutant_submit_dict[mutant.id][mutant_field_dict.DELETE][dict.PREVALUE]?
                                                                                        mutant_status_dict.DELETED:mutant_submit_dict[mutant.id][dict.STATUS]):''}"
                                    on:click={()=>handleChangeCurrentMutantId(mutant.id, mutant.sample.id)}>
                                    <td class="done">
...
<script>
    ...
    function changeMutantParamDone() {
