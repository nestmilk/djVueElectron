            ...
            <div class="navLeft">
                ...
                <div class="leftTopMidWrapper">
                    <div class="messageWrapper">
                        <div class="title">
                            {totalSubmitAndAffirmed}条已提交，{totalUnsubtmitAndAffirmed}条待提交，{totalUnsubtmitAndUnaffirmed}条待审核
                        </div>
                        <div class="uploadButtomWrapper">
                            ...
                            <div class="uploadBnWrapper">
                                <button class="icon-cloud-upload {panal_done || totalUnsubtmitAndAffirmed === 0?'disabled':''}"
                                        disabled="{panal_done || totalUnsubtmitAndAffirmed === 0?'disabled':''}"
                                        on:click={()=>uploadAffirmedMutant()}>&nbsp;提交</button>
                                <button class="icon-file-excel {totalUnsubtmitAndUnaffirmed !== 0 || totalUnsubtmitAndAffirmed !== 0 || panal_done?'disabled':''}"
                                        disabled="{totalUnsubtmitAndUnaffirmed !== 0 || totalUnsubtmitAndAffirmed !== 0 || panal_done?'disabled':''}"
                                        on:click={()=>checkNOUS_AandUS_UAandCreateOutput()}>&nbsp;建表</button>
                                <button class="icon-download {panal_target_url?'':'disabled'}"
                                        disabled="{panal_target_url?'':'disabled'}"
                        ...
                        <table class="down rightMutantTable">
                            {#each mutant_list as mutant, index (mutant.id)}
                                <tr class="{now_mutant_id===mutant.id?'current':''}
                                    ...
                                    on:click={()=>handleChangeCurrentMutantId(mutant.id, mutant.sample)}>
                                    <td class="logs">
                                        ...
                                        {#if mutant_submit_dict[mutant.id]?mutant_submit_dict[mutant.id][dict.REASONDISPLAY]:false}
                                            <div class="reasonWrapper">
                                                ...
                                                <div class="reasaonBnWrapper">
                                                    <button on:click={()=>handleAddReasonSure(mutant.id, mutant.sample)}>确定</button>
                                                    <button on:click={()=>handleAddReasonCancel(mutant.id)}>取消</button>
                                                </div>
                                            </div>
                                        {/if}
                                    </td>
...
<script>
    ...
    let dict = {BAM: 'bam', BAI: 'bai', TARGET: 'target', 'HEREDITARY': 'hereditary', 'TMB': 'TMB',
    ...
    let log_reason_type_dict = {false_positive: '假阳性', other: '其它'}
    ...
    // 未提交已审核 总数 US_AMUT
    let totalUnsubmitAndAffirmed = 0
    // 未提交未审核 总数 US_UAMUT
    let totalUnsubmitAndUnaffirmed = 0
    // 已提交已审核 总数 S_AMUT
    let totalSubmitAndAffirmed = 0
    ...
    async function getSamplesList(panal_id) {
        ...
        await api.retrievePanal(panal_id).then((response) => {
            ...
            mutant_total_num = mutant_list_whole_panal.length
            panal_done = response.data.done
            panal_target_url = response.data.target_url
            // 遍历获取新数组
        ...
        for (let mutant of mutant_list){
            if (mutant.logs.length > 0) {
                for (let log of mutant.logs) {
                    log[dict.LOGFIELDDETAIL] = {done: {nowValue: '', preValue: ''},
    ...
    function handleChangeCurrentMutantId (mutant_id, sample_id) {
        ...
        changeLocus()
        // 非同一sample，重新加载
        // console.log(now_sample_id + " " + sample_id)
        if (pre_sample_id !== now_sample_id) loadTracks()
    ...
    function download() {
        if (panal_target_url) window.location.href = panal_target_url
    }
    ...
    async function checkNOUS_AandUS_UAandCreateOutput(){
        ...
        // 创建表
        let success = false
        if (!panal_done && totalUnsubtmitAndAffirmed === 0 && totalUnsubtmitAndUnaffirmed === 0) {
           await api.createExcel(params.panalId, params.type).then((response)=>{
               ...
               panal_target_url = response.data.url
           }).catch((error)=>{
    ...
    async function uploadAffirmedMutant() {
        if (totalUnsubtmitAndAffirmed===0){
    ...
    function addTrackConfigs(files){
        ...
        for (let bam_name in bamFiles) {
            ...
            if (find) {
                ...
                let find = false
                for (let sample of sample_list) {
                    if (bam_name.indexOf(sample.sampleSn) !== -1) {
